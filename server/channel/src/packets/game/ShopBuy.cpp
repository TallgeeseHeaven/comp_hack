...
void HandleShopPurchase(const std::shared_ptr<ChannelServer> server,
  ...
  if (shop && shop->GetType() != objects::ServerShop::Type_t::COMP_SHOP) {
    ...
    // check normal shops
    if (state->GetCurrentMenuShopID((int32_t)SVR_CONST.MENU_SHOP_BUY) !=
        shopID) {
      auto accountUID = state->GetAccountUID();
      LogGeneralError([shopID, accountUID]() {
        return libcomp::String(
                   "Player attempted a purchase at shop %1 which they are not "
  ...
  auto def = product != nullptr
                 ? definitionManager->GetItemData(product->GetItem())
                 : nullptr;
  if (!shop || !product || !def) {
    LogGeneralError([shopID, productID]() {
      return libcomp::String("Invalid shop purchase: shopID=%1, productID=%2\n")
          .Arg(shopID)
          .Arg(productID);
    });
  ...
  for (uint8_t i = 0; i < (uint8_t)shop->TabsCount(); i++) {
    ...
    for (auto p : tab->GetProducts()) {
      if (p->GetProductID() == productID) {
        ...
        break;
      } else if (!cpPurchase && trendAdjust > 0.f && !p->GetTrendDisabled() &&
        ...
        auto p2 = definitionManager->GetShopProductData(p->GetProductID());
        auto def2 =
            p2 ? definitionManager->GetItemData(p2->GetItem()) : nullptr;
  ...
  if (!productFound) {
    LogGeneralError([shopID, productID]() {
      return libcomp::String(
                 "Shop '%1' does not currently contain product '%2'\n")
          .Arg(shopID)
          .Arg(productID);
    });
  ...
  if (!cpPurchase) {
    ...
    if (trendAdjust > 0.f) {
      ...
      switch (trend) {
        ...
        case 2:
          // Decreased price
          price = (int32_t)ceil((double)price * (double)(1.f - trendAdjust));
          break;
        case 0:
        default:
          // No price adjustment
    ...
    if (shop->GetLNCAdjust()) {
      ...
      price = (int32_t)ceil((float)price *
                            (lncDelta * 0.00002f + (lncCenter ? 0.8f : 0.9f)));
    }
  }
  ...
  if (!cpPurchase) {
    ...
    if (!characterManager->CalculateMaccaPayment(
      ...
      LogGeneralError([accountUID]() {
        return libcomp::String(
                   "Attempted to buy an item the player could not afford: %1\n")
            .Arg(accountUID.ToString());
      });
    ...
    if (qtyLeft < maxStack) {
      for (auto item : characterManager->GetExistingItems(
               character, product->GetItem(), inventory)) {
        if (qtyLeft == 0) break;
        ...
        uint16_t stackAdd =
            (uint16_t)((qtyLeft <= stackLeft) ? qtyLeft : stackLeft);
    ...
    while (qtyLeft > 0) {
      ...
      insertItems.push_back(
          characterManager->GenerateItem(product->GetItem(), stack));
    ...
    if (characterManager->UpdateItems(client, false, insertItems,
      ...
      auto accountUID = state->GetAccountUID();
      LogItemDebug([productID, quantity, price, accountUID]() {
        return libcomp::String(
                   "Shop product %1 (x%2) purchased for %3 macca by player: "
                   "%4\n")
            .Arg(productID)
            .Arg(quantity)
            .Arg(price)
    ...
    if (!gifteeName.IsEmpty()) {
      ...
      targetCharacter =
          objects::Character::LoadCharacterByName(worldDB, gifteeName);

      if (targetCharacter == nullptr) {
        LogGeneralError([gifteeName]() {
          return libcomp::String("Invalid gift target character name: '%1'\n")
              .Arg(gifteeName);
        });
    ...
    if (!lobbyDB->ProcessChangeSet(opChangeset)) {
      LogGeneralError([account]() {
        return libcomp::String(
                   "Attempted to buy an item exceeding the player's CP amount: "
                   "%1\n")
      ...
      if (!gifteeName.IsEmpty()) {
        LogItemDebug([productID, price, gifteeName, account]() {
          return libcomp::String(
                     "Shop product %1 purchased for %2 CP as a gift for '%3' "
                     "by player: %4\n")
              .Arg(productID)
              .Arg(price)
              .Arg(gifteeName)
        ...
        LogItemDebug([productID, price, account]() {
          return libcomp::String(
                     "Shop product %1 purchased for %2 CP by player: %3\n")
              .Arg(productID)
              .Arg(price)
              .Arg(account->GetUUID().ToString());
        });
